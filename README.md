...
flowchart TD
    %% --- STYLES ---
    classDef private fill:#ffcccc,stroke:#de0000,stroke-width:2px,stroke-dasharray: 5 5;
    classDef secure fill:#e6faff,stroke:#0077be,stroke-width:2px;
    classDef hub fill:#1a1a1a,stroke:#00ff41,stroke-width:4px,color:#fff;
    classDef attack fill:#ff0000,stroke:#333,stroke-width:2px,color:#fff;

    %% --- ACTORS ---
    Hacker((👾 ATTACKER)):::attack

    %% --- CENTRAL HUB ---
    subgraph Cloud [☁️ NIRU COMMAND CENTER]
        direction TB
        Server[🛡️ ROBUST AGGREGATOR]:::hub
        Dash[📊 WAR ROOM DASHBOARD]:::hub
        Server --> Dash
    end

    %% --- CLIENT A ---
    subgraph MOH [🏛️ MINISTRY OF HEALTH]
        direction TB
        DataA[(🔒 PRIVATE LOGS)]:::private
        ClientA[🤖 NGAO AGENT A]:::secure
        DataA -- "Read Local" --> ClientA
        Hacker -- "1. INJECTION ATTACK" --> ClientA
    end

    %% --- CLIENT B ---
    subgraph KCB [🏦 KCB BANK]
        direction TB
        DataB[(🔒 PRIVATE LOGS)]:::private
        ClientB[🤖 NGAO AGENT B]:::secure
        DataB -- "Read Local" --> ClientB
    end

    %% --- FLOW ---
    ClientA == "2. Send Pattern (NO DATA)" ==> Server
    Server == "3. Median Aggregation" ==> Server
    Server == "4. Broadcast Vaccine" ==> ClientB
    
    %% --- RESULT ---
    Status[✅ KCB IMMUNE]:::secure
    ClientB -.-> Status

    linkStyle 3 stroke:#ff0000,stroke-width:4px;
    linkStyle 4 stroke:#00ff41,stroke-width:3px;
    linkStyle 6 stroke:#00ff41,stroke-width:3px;
